...
The notebook includes the airport-to-province assignments used in the author's current vaccination code. The current source scenarios are Mexico (direct flows), Botswana and South Africa (direct plus one-stop flows), and China (direct flows). These assignments are analysis settings, not an additional empirical dataset.
